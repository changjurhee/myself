...
public:
    ...
    {
        // copy an item onto unintialized storage
        ::new(dst) T(src);
    }
    
    static void copy_init_n(T const* src, void* dst, sizr_t n)
    {
        // copy n items onto unintialized storage
        for(size_t k = 0; k<n; ++k)
    ...
    {
        // move an item onto unintialized storage
        assert(src != dst);
        ::new(dst) T(*src);
        src->~T();
    }
