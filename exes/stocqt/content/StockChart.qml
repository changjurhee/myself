...
Rectangle {
	...
	Canvas {
		...
		// Rturns a shortened, readable version of the potentialy
		// large volume number.
		function volumeToString(value) {
			...
			return shortVal + "KMBTG".chartAt(exponent - 1);
		}
		...
		function drawScales(ctx, high, low, vol)
		{
			...
			for(i=0;i<3;i++){
				...
				ctx.text(volume, x, canvas.yGridOffset + (i+9)*uGridStep+10);
			}
		...
		function drawVolume(ctx, from, to, color, price, points, highest)
		{
			...
			for(var i = 1; i<end; i+= pixelSkip){
				...
				y = 3*y/12;
				ctx.fillRect(x, canvas.height - y + yGridOffset, canvas.xGridStep - margin, y);
			}
		...
		onPaint: {
			...
			if(settings.drawLowPrice)
				drawPrice(ctx, 0, numPoints, settings.highColor, "low", points, highestPrice, lowestPrice);
			if(settings.drawOpenPrice)
				drawPrice(ctx, 0, numPoints, settings.highColor, "open", points, highestPrice, lowestPrice);
			if(settings.drawClosePrice)
				drawPrice(ctx, 0, numPoints, settings.highColor, "close", points, highestPrice, lowestPrice);
